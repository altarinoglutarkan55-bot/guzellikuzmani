...
export default function AppHeader() {
  ...
  // mağazadaysak q paramını input’a senkron tut
  ...
  const pushMagazaWith = (mutate: (qs: URLSearchParams) => void) => {
    // /magaza dışındaysak temiz başla, /magaza'daysak mevcut query'yi koru
  ...
  const submit = (e: React.FormEvent) => {
    ...
    pushMagazaWith((qs) => {
      ...
      // sort yoksa popüler olsun (daha stabil görünür)
  ...
  const goCategory = (kat: string) => {
    pushMagazaWith((qs) => {
      ...
      // kategori değişince sayfa daha tutarlı: aramayı KORU, anket filtrelerini KORU (c=)
      if (!qs.get("sort")) qs.set("sort", "pop");
    });
  };
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-zinc-200 bg-white/85 backdrop-blur">
      <div className="mx-auto max-w-screen-xl px-4">
        ...
        {/* KATEGORİ SLIDER (HER ZAMAN GÖRÜNÜR) */}
        <nav className="pb-3">
